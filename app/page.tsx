...
function Logo(){
  return(
    <div className="flex gap-2 items-center">
      <div style={{backgroundColor: mainColor}} className={`p-[3px] rounded-md`}>
        <TextSnippetIcon sx={{fontSize:40,color:"white"}}/>
      </div>
      <div className="flex gap-1 text-[19px]">
        <span style={{color:mainColor}} className={`font-bold`}>Code</span>
...
function Button(){
  return(
    <div className="flex gap-2 max-sm:flex-col max-sm:w-[60%] max-sm:mt-8">
      ...
      <button style={{borderColor:mainColor, color:mainColor,}} className={`text-sm border border-[${mainColor}] text-[${mainColor}]\ hover:bg-[${mainColor}] hover:text-white p-[8px] px-6 rounded-md`}>
        Sign Up
      </button>
    </div>
  )
...
function CTASection(){
  return(
    <div className="flex flex-col mx-16 items-center mt-[120px] gap-6">
      ...
      <button className={`block px-9 py-3 text-sm font-medium text-white transition focus:outline-none`}
      type="button">
        {`Let's get started!`}
      </button>
    </div>
